...
@push('script')
    <script type="text/javascript">
        ...
        $(document).on("change", ".transfer-currency", function () {
            ...
            var output = null;
            $('#showInfo').html('');
            output = `<li class="list-group-item font-weight-bold list-text bg-transparent customborder text-white">@lang('Exchange rate') :
                        <span
                            class="float-right text-danger">1 ${baseCurrency} = ${rate} ${currencyCode}</span>
                    </li>
                    <li class="list-group-item font-weight-bold list-text bg-transparent customborder text-white">@lang('You will get:') :
                        <span
